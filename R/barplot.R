...
barplot_rd3 <- function(nodes, links, name = NULL, select = NULL,
  ...
  notListed <- length(setdiff(union(links[[source]],links[[target]]),nodes[[name]]))
  if(notListed!=0){
    stop(paste(notListed," node link(s) not defined in '",name,"' column from nodes data frame."))
  }
  ...
  if(!is.null(sort)){
      if(sort %in% colnames(nodes)){
        options[["order"]] <- sort
        options[["rev"]] <- bitwXor(as.integer(is.numeric(nodes[,sort])),as.integer(decreasing))
      }else{
  ...
  bar <- structure(list(nodes=nodes, links=links, options=options), class="barplot_rd3")
